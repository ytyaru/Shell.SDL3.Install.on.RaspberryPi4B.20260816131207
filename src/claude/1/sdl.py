...
想定環境: Raspberry Pi 4B (aarch64, Debian系, 2コア使用してビルド)

このスクリプトは「手動作業を一つも残さない」ことを目標に、
ビルドに必要なOS側の依存パッケージ (apt) の確認・導入までを自動で行う。
...
設計方針 (単一責任の原則に従いクラスを分割):
  ...
  DependencyInstaller: OS側の依存パッケージ(apt)が導入済みか確認し、
                       未導入のものだけをインストールする責務のみを持つ。
                       (ビルド方法やSDL自体の知識は持たない)
  ResourceAdvisor    : ラズパイのメモリ/スワップ量を見て、ビルド前に
                       警告を出す責務のみを持つ(実処理は行わない)。
  GitHubRelease      : GitHub API から各リポジトリの最新リリースタグ/バージョンを
                       取得する責務のみを持つ。
  SourceFetcher      : ソースZIPのダウンロードと展開 (インクリメンタル) の責務のみ。
  Builder            : Ninja + cmake による configure / build / install の責務のみ。
  ...
  Prompter (confirm) : Y/n 確認プロンプト表示の責務のみ。
  Cli                : サブコマンドの解釈と、上記クラスの呼び出しのみを行う
                       (ビジネスロジックは持たない)。
--------------------------------------------------------------------

★ 重要な設計判断とユーザーへの確認事項 (このスクリプトの末尾のREADME_NOTESも参照):

  1. GitHubの「Download ZIP」アーカイブには git submodule の中身が含まれない。
     SDL_image/SDL_ttf/SDL_mixer は本来 external/ 配下に依存ライブラリを
     submoduleとして持つ構成 (VENDORED=ON) が多いが、ZIP展開ではそれが
     空ディレクトリになりビルドできない。
     → そのため本スクリプトは VENDORED を使わず、OS側にシステムライブラリ
       (libpng-dev, libfreetype-dev, libmpg123-dev 等) をaptで導入し、
       それらをcmakeにリンクさせる方式に変更した。
  2. SDL_mixerのMP3バックエンドは mpg123 (システムライブラリ) を採用した。
     vendored な drmp3 (ヘッダオンリー実装) はSDL_mixerバージョンによって
     submodule経由か同梱かが変わり不確実なため、確実に動くmpg123を選んだ。
  3. IMEによる日本語入力は「ビルド依存」としては libibus-1.0-dev の導入までを
     本スクリプトが担当する。ただし実際に日本語入力するには fcitx5-mozc 等の
     IME本体(実行時アプリケーション)がデスクトップ環境で動作している必要があり、
     これはビルド依存パッケージとは性質が異なる(ビルドではなく実行環境の話)ため
     本スクリプトでは自動導入していない。必要なら別途
     `sudo apt-get install fcitx5 fcitx5-mozc` 等を検討してほしい。
  4. X11とWayland両方の開発ヘッダをaptで導入する構成にした
     (どちらの環境で動かすか事前にわからないため)。片方に絞りたい場合は
     DEPENDENCY_PACKAGES の該当行を削ってほしい。
  5. apt-get の実行には root権限が必要。root でなければ sudo 経由で実行する。
     sudoのパスワード入力が必要な環境では、そこだけ非対話にはできない
     (要件通りの「全自動」を貫くなら NOPASSWD sudo 設定を推奨)。
--------------------------------------------------------------------
"""
...
# ---------------------------------------------------------------------------
# 依存パッケージ一覧 (apt)。用途ごとにグループ化しコメントしている。
# 名前が Debian のバージョンによって揺れるものは候補リストで表現し、
# DependencyInstaller が apt-cache で存在確認して解決する。
# ---------------------------------------------------------------------------

# 確実に名前が一定しているパッケージ
APT_PACKAGES_FIXED: List[str] = [
    # --- ビルドツール本体 ---
    "build-essential",
    "cmake",
    "ninja-build",   # 高速+省メモリビルドのため make ではなく ninja を使う
    "pkg-config",
    "git",
    # --- SDL3本体: 音声/入力/ウィンドウシステム全般 ---
    "libasound2-dev",       # ALSA (音声)
    "libpulse-dev",         # PulseAudio (音声)
    "libx11-dev",
    "libxext-dev",
    "libxrandr-dev",
    "libxcursor-dev",
    ...
    "libdbus-1-dev",
    "libudev-dev",
    "libibus-1.0-dev",      # IME(ビルド時ヘッダ。実行時IME本体は別途)
    # --- SDL_image: PNG ---
    "zlib1g-dev",
    "libpng-dev",
    # --- SDL_ttf: 日本語フォント表示 ---
    "libharfbuzz-dev",
    # --- SDL_mixer: MP3 (mpg123バックエンド) ---
    "libmpg123-dev",
]

# Debianのバージョンで名前が変わるもの: 候補を先頭から順に apt-cache で存在確認する
APT_PACKAGES_ALTERNATIVES: List[List[str]] = [
    ["libfreetype-dev", "libfreetype6-dev"],  # SDL_ttf用 (新旧Debianで名前が違う)
]
...
# ============================================================================
# DependencyInstaller: OS側の依存パッケージ(apt)の確認/導入のみを担当
# ============================================================================

class DependencyInstaller:
    def __init__(self) -> None:
        self.apt_available = shutil.which("apt-get") is not None
        self._resolved_packages: Optional[List[str]] = None

    # ------------------------------------------------------------ 名前解決
    def _resolve_alternative(self, candidates: List[str]) -> Optional[str]:
    ...
    def resolve_packages(self) -> List[str]:
        if self._resolved_packages is not None:
            return self._resolved_packages
        packages = list(APT_PACKAGES_FIXED)
        for candidates in APT_PACKAGES_ALTERNATIVES:
            resolved = self._resolve_alternative(candidates)
            if resolved is None:
                err(
                    "実装エラー: 次の候補のいずれもaptパッケージ一覧に見つかりません: "
                    + ", ".join(candidates)
                    + " (パッケージ名が変わった可能性があります。手動確認してください)"
                )
                continue
            packages.append(resolved)
        self._resolved_packages = packages
        return packages
    ...
    def missing_packages(self) -> List[str]:
        return [p for p in self.resolve_packages() if not self._is_installed(p)]

    # ------------------------------------------------------------ 導入本体
    def ensure_installed(self, auto: bool = False) -> None:
        if not self.apt_available:
            err(
                "apt-get が見つかりません。Debian系以外の環境と思われるため、"
                "依存パッケージの自動導入をスキップします。"
                "手動で以下相当のパッケージを導入してください: "
                + ", ".join(self.resolve_packages())
            )
            return

        missing = self.missing_packages()
        if not missing:
            log("依存パッケージは全て導入済みです。")
            return

        print("以下の依存パッケージが未導入のため導入します:")
        for p in missing:
            print(f"  {p}")
        if not confirm("apt-get でインストールします。よろしいですか？", auto):
            die("依存パッケージが未導入のため中止します。")

        sudo_prefix = [] if os.geteuid() == 0 else ["sudo"]
        run(sudo_prefix + ["apt-get", "update"])
        env = dict(os.environ)
        env["DEBIAN_FRONTEND"] = "noninteractive"
        run(sudo_prefix + ["apt-get", "install", "-y"] + missing, env=env)
        log("依存パッケージの導入が完了しました。")


# ============================================================================
# ResourceAdvisor: メモリ/スワップの警告表示のみを担当 (処理はしない)
# ============================================================================
    ...
    def _read_meminfo() -> Dict[str, int]:
        ...
        try:
            for line in Path("/proc/meminfo").read_text(encoding="utf-8").splitlines():
                ...
                info[key] = int(value)  # kB単位
        except Exception:
            pass
        return info

    def warn_if_low_memory(self) -> None:
        info = self._read_meminfo()
        if not info:
            return
        total_kb = info.get("MemTotal", 0)
        swap_kb = info.get("SwapTotal", 0)
        total_gb = total_kb / (1024 * 1024)
        swap_gb = swap_kb / (1024 * 1024)

        # SDL_ttf(freetype+harfbuzz)やSDL_mixer等はninja並列ビルド時にメモリを
        # 多く使うコンパイル単位がある。目安として (RAM+Swap) が3GB未満なら警告する。
        if total_gb + swap_gb < 3.0:
            log(
                f"警告: 搭載メモリ約{total_gb:.1f}GB + スワップ約{swap_gb:.1f}GB は "
                "並列ビルド(2並列)には余裕が少ない可能性があります。"
                "ビルドが途中で強制終了する場合はスワップ増設を検討するか、"
                "BUILD_JOBS定数を1に下げてください。"
            )
...
# ============================================================================
# Builder: Ninja + cmake による configure/build/install のみを担当
# ============================================================================
...
class Builder:
    ...
    def _extra_cmake_args(key: str) -> List[str]:
        ...
        GitHubのZIP展開にはgit submoduleが含まれないため VENDORED は使わず、
        aptで導入したシステムライブラリにリンクする方式にしている。"""
        if key == "sdl":
            return []
        if key == "image":
            return [
        ...
        if key == "mixer":
            return [
                ...
                # 要件はMP3のみのため他形式は明示的に絞る(依存追加を避ける)
    ...
    @staticmethod
    def _needs_reconfigure(build_dir: Path) -> bool:
        """以前 Makefiles 等 別ジェネレータで configure 済みの build_dir が
        残っている場合、Ninjaへ切り替えるにはキャッシュを作り直す必要がある。"""
        cache = build_dir / "CMakeCache.txt"
        if not cache.exists():
            return False
        text = cache.read_text(encoding="utf-8", errors="ignore")
        return "Ninja" not in text
    ...
    def build_and_install(
        ...
        if build_dir.exists() and self._needs_reconfigure(build_dir):
            log(f"{key} {version}: 生成方式(ジェネレータ)が変わったためbuildディレクトリを作り直します")
        ...
        # 既にbuild_dirにNinja用CMakeCache.txtがあればインクリメンタルconfigureになる
        run(configure_cmd)
...
HELP_MAIN = """SDLの最新安定版をローカル内で管理する。
コマンド一覧:
  help [コマンド]  ヘルプを表示する。
  install          インストールする。
...
説明: 依存パッケージ(apt)の確認・導入 → GitHubにあるZipからソースコードを
      ダウンロード → Ninja+cmakeでビルド → インストール、までを全自動で行う。
      ラズパイ4B(4GB)で動作させることを想定している。
用法: sdl.py install [auto]
引数:
  auto   付与すると依存パッケージのapt導入を含め確認せず強制実行する。
""",
...
class Cli:
    ...
    def cmd_install(self, args: List[str]) -> None:
        ...
        # 0. 依存パッケージ(apt)の確認・導入。未導入分のみ導入する。
        DependencyInstaller().ensure_installed(auto=auto)
        ResourceAdvisor().warn_if_low_memory()
        ...
        # 4. 依存順にビルド。既にインストール済みのバージョンはBuilder側でスキップされる。
