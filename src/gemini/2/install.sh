...
sudo apt-get install -y \
    ...
    libasound2-dev libpulse-dev libaudioresource-dev \
...
# GitHubから最新の安定版タグ（偶数マイナーバージョン 例: 3.2.x）を取得する関数
...
build_and_install() {
    ...
    if [ -z "$tag" ]; then
        # 万が一API制限などで取得できなかった場合のフォールバック（最新のタグを取得）
        tag=$(curl -s "https://github.com{repo}/tags" | grep -oP '"name": "\K[^"]+' | head -n 1)
    fi
    ...
    # --fdescで説明を付与、--providesでpkg-configが要求する名前を補完
    sudo checkinstall -y \
        --pkgname="${pkg_name}" \
        --pkgversion="${tag#v}" \
        --pkgrelease="1" \
