...
void cleanup_all(SDL_Window* window, SDL_Renderer* renderer, TTF_Font* font, SDL_Texture* texture) {
...
int main(int argc, char* argv[]) {
    ...
    if (!(IMG_Init(IMG_INIT_PNG) & IMG_INIT_PNG)) {
        std::cerr << "SDL3_image 初期化失敗: " << SDL_GetError() << std::endl;
        SDL_Quit();
        return 1;
    }
    if (!TTF_Init()) {
    ...
    if (!window) {
        ...
        cleanup_all(nullptr, nullptr, nullptr, nullptr);
        return 1;
    }
    SDL_Renderer* renderer = SDL_CreateRenderer(window, nullptr);
    if (!renderer) {
        std::cerr << "レンダラー生成失敗: " << SDL_GetError() << std::endl;
        cleanup_all(window, nullptr, nullptr, nullptr);
        return 1;
    }
    ...
    cleanup_all(window, renderer, font, textTexture);
    return 0;
}
