...
int main(int argc, char* argv[]) {
    ...
    // SDL3_image: SDL3では IMG_Init は必須ではなくなり、必要に応じて自動初期化されますが、
    // リンク確認を兼ねて有効なヘッダー・ライブラリ接続テストとして進めます。
    ...
    // SDL3_mixer: 引数が仕様変更され、デバイスIDとスペックを渡す形になりました
    ...
    // SDL3_net: プレフィックスが SDLNet_ から NET_ へ変更されました
    if (NET_Init() < 0) {
        ...
        while (SDL_PollEvent(&event)) {
            if (event.type == SDL_EVENT_QUIT) {
                ...
                if (SDL_strlen(event.edit.text) > 0) {
                    std::cout << "[IME 変換中...]: " << event.edit.text << std::endl;
                }
            }
        }
